...
class EditNoteUseCase:
    ...
    async def execute(self, note_id: int, new_local_encrypted_content: str, new_client_private_key_b64: Optional[str] = None,new_title:Optional[str]=None,new_tags:Optional[str]=None) -> str:
        ...
        new_encrypted_title =  self.encryption.encryptserver(new_title) if new_title is not None else existing_note.title
        ...
        updated_note = await self.repo.update_notes(note_id, encrypted_content,existing_note.title,new_tags,existing_note.created_at)
