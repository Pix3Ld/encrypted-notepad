...
class InMemoryTrashRepository(TrashRepository):
    ...
    async def restore_note_from_trash(self, note_id: int) -> Optional[Note]:
        ...
        if restored:
            ...
            note = Note(id=restored.id, content=restored.content,created_at=restored.created_at, key_private_b64=restored.key_private_b64,title=restored.title,tags=restored.tags,)
            return note
        return None
